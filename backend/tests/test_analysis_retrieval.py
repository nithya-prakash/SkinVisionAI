...
async def _upload(client: AsyncClient, image_bytes: bytes, session_id: str | None = None) -> dict:
...
@pytest.mark.asyncio
async def test_get_analysis_linked_to_its_session(client: AsyncClient) -> None:
    session_id = (await client.post("/api/sessions")).json()["id"]
    upload = await _upload(client, to_bytes(make_acceptable_image(800, 800), "JPEG"), session_id)

    response = await client.get(f"/api/analysis/{upload['analysis_id']}")
...
@pytest.mark.asyncio
async def test_get_analysis_ready_for_visual_analysis(client: AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_get_analysis_quality_rejected(client: AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_get_analysis_completed_after_visual_analysis(client: AsyncClient) -> None:
    upload = await _upload(client, to_bytes(make_acceptable_image(800, 800), "JPEG"))
    ran = await client.post(f"/api/analysis/{upload['analysis_id']}/visual-analysis")
...
@pytest.mark.asyncio
async def test_get_analysis_unknown_id_returns_404(client: AsyncClient) -> None:
    response = await client.get(f"/api/analysis/{uuid.uuid4()}")
    assert response.status_code == 404
    assert response.json()["detail"]["code"] == "analysis_not_found"


@pytest.mark.asyncio
async def test_get_analysis_repeated_retrieval_is_stable(client: AsyncClient) -> None:
    upload = await _upload(client, to_bytes(make_acceptable_image(800, 800), "JPEG"))
    await client.post(f"/api/analysis/{upload['analysis_id']}/visual-analysis")

    first = await client.get(f"/api/analysis/{upload['analysis_id']}")
    second = await client.get(f"/api/analysis/{upload['analysis_id']}")
    assert first.json()["visual_analysis"] == second.json()["visual_analysis"]
    assert first.json()["status"] == second.json()["status"] == "completed"


@pytest.mark.asyncio
async def test_get_analysis_never_exposes_a_filesystem_path(client: AsyncClient) -> None:
    upload = await _upload(client, to_bytes(make_acceptable_image(800, 800), "JPEG"))
    await client.post(f"/api/analysis/{upload['analysis_id']}/visual-analysis")
    response = await client.get(f"/api/analysis/{upload['analysis_id']}")
...
@pytest.mark.asyncio
async def test_get_analysis_invalid_id_format_returns_422(client: AsyncClient) -> None:
