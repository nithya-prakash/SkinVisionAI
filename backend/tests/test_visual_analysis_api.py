...
async def _upload(authenticated_client: AsyncClient, image_bytes: bytes, filename: str = "photo.jpg") -> dict:
    response = await authenticated_client.post(UPLOAD_URL, files={"file": (filename, image_bytes, "image/jpeg")})
...
async def test_visual_analysis_on_accepted_image_returns_structured_result(
    ...
    response = await authenticated_client.post(_visual_analysis_url(upload["analysis_id"]))
...
@pytest.mark.asyncio
async def test_visual_analysis_rejects_low_quality_image_with_422(authenticated_client: AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_visual_analysis_rejects_tiny_image_with_422(authenticated_client: AsyncClient) -> None:
    upload = await _upload(authenticated_client, to_bytes(make_tiny_image(50, 50), "JPEG"))
    response = await authenticated_client.post(_visual_analysis_url(upload["analysis_id"]))
    assert response.status_code == 422


@pytest.mark.asyncio
async def test_visual_analysis_unknown_analysis_id_returns_404(authenticated_client: AsyncClient) -> None:
    response = await authenticated_client.post(_visual_analysis_url(str(uuid.uuid4())))
...
async def test_visual_analysis_invalid_analysis_id_format_returns_422(
    ...
    response = await authenticated_client.post(_visual_analysis_url("not-a-uuid"))
    assert response.status_code == 422


@pytest.mark.asyncio
async def test_visual_analysis_never_exposes_a_filesystem_path(authenticated_client: AsyncClient) -> None:
    upload = await _upload(authenticated_client, to_bytes(make_acceptable_image(800, 800), "JPEG"))
    response = await authenticated_client.post(_visual_analysis_url(upload["analysis_id"]))
...
@pytest.mark.asyncio
async def test_visual_analysis_rejects_when_image_was_not_retained(authenticated_client: AsyncClient) -> None:
...
async def test_visual_analysis_is_deterministic_across_repeated_calls(
    ...
    first = await authenticated_client.post(_visual_analysis_url(upload["analysis_id"]))
    second = await authenticated_client.post(_visual_analysis_url(upload["analysis_id"]))
