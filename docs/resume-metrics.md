...
application — on 2026-09-09, immediately after the release-hardening
pass. None is estimated, rounded up, or inferred. "How measured" says
...
|---|---|---|---|---|---|
| 1 | Backend test suite size | **756 tests** | Total unit/integration tests in `backend/tests/` | Tests | `pytest --collect-only` count, confirmed via `docker compose exec api pytest tests/ -q` |
| 2 | Backend test pass rate | **756/756 = 100%** (full checkout) | Whether the entire backend suite passes | Tests / live verification | Full re-run against a freshly rebuilt (`--no-cache`) Docker image and an empty database. (755/756 pass with 1 environment-conditional skip when run *inside* the backend-only Docker image, because that one test cross-checks `README.md` against the rule data and the Docker build context doesn't copy `README.md` in — it runs and passes in a full local checkout.) |
...
| 16 | Frontend application routes | **7** (`/`, `/analyze`, `/chat`, `/compare`, `/history`, `/results/[id]`, `/routine`) | Distinct user-facing pages | Live verification | `next build` route output against the current build |
| 17 | API endpoints | **19** REST endpoints under `/api/` (plus 1 health check) | Size of the backend's public HTTP surface | Live verification | `GET /openapi.json` against the running API, counted programmatically |
| 18 | Database tables | **12** application tables (plus Alembic's own bookkeeping table) | Persistent schema size | Live verification | `\dt` against a freshly migrated, empty PostgreSQL database |
| 19 | Database migrations | **5** versioned Alembic migrations | Schema evolution history, all applying cleanly from empty | Source code / live verification | File count in `backend/alembic/versions/`, confirmed applying with zero errors on a fresh `alembic upgrade head` |
...
| 25 | Rate-limited endpoints | **2** (image upload: 10 req/60s; agent chat: 20 req/60s, both per client IP, configurable) | Cost-bearing endpoints protected against abuse in an unauthenticated app | Source code / live verification | `app.core.rate_limit` config defaults; a live test hammering the endpoint past its limit returned a controlled `429` |
| 26 | Development phases completed | 12 (context only — see note below) | Project scope/history, not an engineering achievement | Source code / docs | `docs/phases.md` build log |

**Note on #26**: phase count describes how the project was built, not
...
1. **756 backend tests, 100% passing** — the single most concrete
...
7. **5-feature classical computer vision pipeline, fully deterministic
   ...
   parts of the stack.
8. **19 REST endpoints, 12-table PostgreSQL schema, 5 migrations, all
...
10. **2 rate-limited endpoints with per-IP fixed-window limiting** — a
...
- Designed and built a full-stack AI application enforcing a strict
  **deterministic/LLM trust boundary**: all domain facts (CV
  ...
  orchestrates and narrates — backed by a **756-test suite** and a
