...
boundary, prompt-injection handling, upload safety, error handling, and
the honest limitations of an anonymous, no-auth demo app. Each section
...
`app/core/rate_limit.py` (Phase 12 follow-up): a hand-rolled, per-client-
IP fixed-window limiter on the two endpoints with a real per-request
cost in an app with no authentication to otherwise bound abuse —
`POST /api/analysis/upload` (CV pipeline CPU) and `POST /api/agent/chat`
(real LLM provider token cost). Default limits: 10 uploads/minute, 20
chat messages/minute per IP (`RATE_LIMIT_*` in `.env.example`). A
request over the limit gets a controlled `429` with the same
`{code, message}` shape every other error uses, plus a `Retry-After`
header — never a bare rejection.
...
## Session / anonymous-access limitations (stated plainly)

This application has **no authentication**, by explicit design across
every phase, including this one. A session id, chat-session id, or any
other UUID-addressable resource id is a plain, non-secret identifier —
**possessing the id grants access to it**, exactly like every other
UUID-addressable resource in this app (a product id, an analysis id).
This means:

- Session A can read or link to Session B's analysis/product/routine/
  comparison/chat purely by knowing (or guessing) its UUID. There is no
  ownership check anywhere, and none is added by this phase — adding one
  requires authentication, which is explicitly out of scope.
- A malformed (non-UUID) `session_id`/`chat_session_id` on a **write**
  path (uploading an image, sending a chat message) is silently
  discarded and a fresh session/chat is created instead of erroring —
  this matches `get_or_create_session`'s existing, consistent semantics
  for "no id was usably supplied," the same way omitting the id entirely
  already behaves. Every **read** path
  (`GET /api/sessions/{id}`, `GET /api/analysis/{id}`,
  `GET /api/chat/sessions/{id}`, and all five session-list endpoints)
  404s cleanly for both a malformed and an unknown id.
- No double-submission/idempotency protection exists: clicking
  ...
  two independent history rows. At this application's single-user,
  no-payment, demo scope this is cosmetic (an extra history entry), not
  a correctness or safety problem, and no idempotency-key mechanism is
  added by this phase — see [Known limitations](#known-limitations).

This is not a regression or an oversight introduced by Phase 10 — it is
the same access model documented since Phase 1 (see
[docs/persistence.md](persistence.md#security)) and
[frontend/src/lib/session.ts](../frontend/src/lib/session.ts)'s own doc
comment) — restated here in one place because Phase 10 is explicitly the
phase that audits for exactly this kind of scattered-but-correct
documentation.
...
- **No image deletion/TTL job.** `IMAGE_RETENTION_MODE=temporary`
  (the default) writes an uploaded image to disk and nothing ever
  deletes it automatically; `=none` never writes it in the first place.
  Building an actual cleanup job is out of scope for this phase (it
  would be new functionality, not integration/safety hardening of what
  exists) — see [docs/vision.md](vision.md#image-privacy-and-retention-phase-2).
- **No authentication / no cross-session isolation.** See above.
